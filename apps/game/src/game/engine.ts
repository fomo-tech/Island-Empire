...
export function createIslandEmpireGame(
  ...
  const BIOMES = [
    ...
    { id: "violet", a: "#3cc5f0", b: "#1fa0cc", hi: "#80ebff", dark: "#11688c", edge: "#084d6b" }, // Bright Turquoise / Sky Blue magic land
    { id: "rose", a: "#ff9736", b: "#d9751e", hi: "#ffc385", dark: "#9e4e0b", edge: "#733704" }, // Bright Vibrant Autumn Orange land
  ...
  const OWNER_BIOMES = [
    ...
    { a: "#3cc5f0", b: "#1fa0cc", hi: "#80ebff", dark: "#11688c", edge: "#084d6b" }, // Purple (4) - bright turquoise overlay
    { a: "#ff9736", b: "#d9751e", hi: "#ffc385", dark: "#9e4e0b", edge: "#733704" }, // Pink (5) - bright orange overlay
  ...
  function townPopulationCap(town: any) {
    ...
    return 64 + lvl * 36 + fort * 24 + warehouse * 8;
  }
  ...
  function cancelClearingIfTargetTaken(regionIds: number[]) {
    ...
    if (derivedRegionOwnership(travel.targetRegionId) !== 0) {
      refundBuildCostForRegion(travel.targetRegionId);
  ...
  function territoryBuildCost(regionId: number) {
    ...
    const yieldGold = y.gold * areaFactor * mult.gold;
    const yieldWood = y.wood * areaFactor * mult.wood;
    const yieldStone = y.stone * areaFactor * mult.stone;
    const yieldFood = y.food * areaFactor * mult.food;
    const yieldIron = y.iron * areaFactor * mult.iron;
    const yieldSulfur = y.sulfur * areaFactor * mult.sulfur;
    const yieldGems = y.gems * areaFactor * mult.gems;
    return {
      gold: Math.round(180 + areaFactor * 32 + yieldGold * 92 + yieldGems * 70),
      wood: Math.round(130 + areaFactor * 28 + yieldWood * 66),
      stone: Math.round(125 + areaFactor * 34 + yieldStone * 76 + yieldIron * 28),
      food: Math.round(80 + areaFactor * 18 + yieldFood * 42),
      iron: Math.round(20 + yieldIron * 95 + yieldSulfur * 30),
      gems: Math.round(Math.max(0, yieldGems - 0.28) * 22),
    };
  }
  ...
  function storageCapacity(town: any) {
    const warehouse = town?.buildings?.warehouse || 0;
    const fort = town?.buildings?.fort || 0;
    return 250 + warehouse * 650 + fort * 180 + (town?.lvl || 1) * 120;
  }
  ...
  function territorySpecialResources(regionId: number) {
    ...
    if ((r.biome === 0 || r.biome === 5 || r.biome === 6) && area >= 11000 && regionId % 4 !== 0) specials.push("Bãi ngựa");
    if (r.isIslet || regionId % 5 === 0 || regionId % 7 === 0) specials.push("Bến tàu tự nhiên");
    if (r.biome === 3) specials.push("Mỏ lưu huỳnh");
    if (r.biome === 2 || r.biome === 3) specials.push("Mạch sắt");
    return specials;
  }
  ...
  function loadCamera() {
    try {
      ...
      if (saved.selected !== undefined) state.selected = saved.selected;
      if (saved.selectedRegion !== undefined) state.selectedRegion = saved.selectedRegion;
      clampPan();
  ...
  function drawPalmTree(x: number, y: number, scale?: number) {
    const sc = scale || 1;
    // Base Shadow
    pxRect(x - 12 * sc, y + 16 * sc, 24 * sc, 5 * sc, "rgba(0,0,0,0.25)");

    // Curved Trunk (Pixelated curve for RTS look)
    pxRect(x - 2 * sc, y + 6 * sc, 5 * sc, 12 * sc, "#6e4318");
    pxRect(x - 1 * sc, y - 4 * sc, 5 * sc, 12 * sc, "#80521e");
    pxRect(x + 1 * sc, y - 14 * sc, 4 * sc, 11 * sc, "#a66d2d");
    
    // Trunk Bark texture rings
    pxRect(x - 1 * sc, y + 10 * sc, 4 * sc, 2 * sc, "#4a2a0d");
    pxRect(x, y + 2 * sc, 4 * sc, 2 * sc, "#4a2a0d");
    pxRect(x + 2 * sc, y - 8 * sc, 3 * sc, 2 * sc, "#5c3813");

    const trunkTopX = x + 3 * sc;
    const trunkTopY = y - 14 * sc;

    // Coconuts hanging from top
    pxRect(trunkTopX - 4 * sc, trunkTopY + 2 * sc, 5 * sc, 5 * sc, "#8c5314");
    pxRect(trunkTopX + 1 * sc, trunkTopY + 4 * sc, 4 * sc, 4 * sc, "#a6681c");
    pxRect(trunkTopX - 2 * sc, trunkTopY + 6 * sc, 4 * sc, 4 * sc, "#6e3f0e");

    // Arching Palm Fronds (Lush Green Coconut Leaves)
    const fronds = [
      { dx: -24, dy: 10, c1: "#1d4e22", c2: "#388225", c3: "#5aa63c" },
      { dx: 24, dy: 12, c1: "#1d4e22", c2: "#388225", c3: "#5aa63c" },
      { dx: -18, dy: -12, c1: "#22592a", c2: "#44a350", c3: "#7be88b" },
      { dx: 18, dy: -10, c1: "#22592a", c2: "#44a350", c3: "#7be88b" },
      { dx: -28, dy: -2, c1: "#1b451e", c2: "#2d7528", c3: "#44a350" },
      { dx: 28, dy: -2, c1: "#1b451e", c2: "#2d7528", c3: "#44a350" },
      { dx: 0, dy: -20, c1: "#22592a", c2: "#5aa63c", c3: "#a3f09b" }
    ];
  ...
  function drawLakeInRegion(r, seed, rx, ry) {
    if (hash(seed * 43) < 0.65) return; // Increased probability of lakes from 22% to 35%!
    ...
    // Draw shadow/depth underneath the lake
    fillPath(organicPath(lx + 2, ly + 3, lrx + 5, lry + 4, seed * 2.1 + 1), "rgba(24,38,30,0.48)");
    
    // Lake water color - deeper blue/cyan
    fillPath(lake, "#1d6c8f");
    
    // Draw lighter inner water highlight for transparency depth
    ...
    // Soft foam shine stroke (very light cyan/white)
    ctx.strokeStyle = "rgba(180, 240, 255, 0.4)";
    ctx.lineWidth = 1;
    ctx.stroke();

    // Draw a lilypad (water detail)
  ...
  function drawRiverInRegion(r, seed, rx, ry) {
    if (hash(seed * 19) < 0.60) return; // Increased probability of rivers from 28% to 40%!
    ctx.save();
    ...
    // Sparkling river core highlight
  ...
  function drawRegionTerrain(r, seed, rx, ry, originalBiome) {
    ...
    // Balanced RTS landscape density (~18 items per region, nicely spaced like standard RTS games)
    const count = 18;

    // Gather all elements to be rendered in the region
    const items: Array<{
    ...
    // Sort items by Y ascending so they overlap beautifully (depth sorting)
    ...
    items.forEach((item) => {
      ...
      // 1. Desert Biome (Sa Mạc - High Gold & Stone)
      if (originalBiome === 1) {
        if (pick > 0.60) drawPalmTree(x, y, pick > 0.85 ? 0.72 : 0.52);
        else if (pick > 0.35) drawCoin(x, y, 1.3);
        else if (pick > 0.18) drawMountain(x, y, pick > 0.28 ? 0.48 : 0.34);
        else drawDune(x, y, 0.65);

      // 2. Snow Biome (Băng Tuyết - High Stone & Iron & Gems)
      } else if (originalBiome === 2) {
        if (pick > 0.55) drawRockPile(x, y, pick > 0.75 ? 1.2 : 0.85);
        else if (pick > 0.28) drawMountain(x, y, pick > 0.45 ? 0.52 : 0.38);
        else if (pick > 0.12) drawSnowTree(x, y, 0.55);
        else drawBush(x, y, 0.5);

      // 3. Volcanic Biome (Núi Lửa - High Iron, Coal & Sulfur)
      } else if (originalBiome === 3) {
        if (pick > 0.65) drawVolcano(x, y, pick > 0.85 ? 0.55 : 0.38);
        else if (pick > 0.35) drawMountain(x, y, pick > 0.55 ? 0.52 : 0.38);
        else if (pick > 0.15) drawRockPile(x, y, 1.15);
        else drawBush(x, y, 0.45);

      // 4. Crystal Biome (Băng Tinh / Mỏ Ngọc - High Gems)
      } else if (originalBiome === 4) {
        if (pick > 0.50) drawRockPile(x, y, pick > 0.75 ? 1.25 : 0.9);
        else if (pick > 0.25) drawTree(x, y, 0.62);
        else if (pick > 0.10) drawBush(x, y, 0.55);
        else drawFlower(x, y, 0.8, "#4cf0ff", "#ffffff");

      // 5. Autumn Biome (Rừng Thu - High Gold & Gems)
      } else if (originalBiome === 5) {
        if (pick > 0.55) drawTree(x, y, pick > 0.78 ? 0.72 : 0.55);
        else if (pick > 0.32) drawCoin(x, y, 1.25);
        else if (pick > 0.15) drawMountain(x, y, 0.42);
        else drawFlower(x, y, 0.75, "#ff7c25", "#fff0a0");

      // 6. Pine Forest Biome (Rừng Thông - High Wood & Food)
      } else if (originalBiome === 6) {
        if (pick > 0.40) drawOakTree(x, y, pick > 0.72 ? 0.82 : 0.58);
        else if (pick > 0.20) drawBush(x, y, 0.68);
        else if (pick > 0.08) drawMountain(x, y, 0.42);
        else drawFlower(x, y, 0.7, "#f0f060", "#ffffff");

      // 7. Mint Swamp Biome (Đầm Lầy - High Wood & Food)
      } else if (originalBiome === 7) {
        if (pick > 0.45) drawSwampTree(x, y, pick > 0.75 ? 0.72 : 0.55);
        else if (pick > 0.22) drawBush(x, y, 0.65);
        else if (pick > 0.10) drawMountain(x, y, 0.42);
        else drawFlower(x, y, 0.75, "#60ff90", "#ffffa0");

      // 0. Grassland Biome (Đồng Cỏ - High Food & Wood)
      } else {
        if (pick > 0.45) drawOakTree(x, y, pick > 0.72 ? 0.78 : 0.55);
        else if (pick > 0.22) drawBush(x, y, pick > 0.35 ? 0.72 : 0.54);
        else if (pick > 0.10) drawMountain(x, y, 0.45);
        else drawFlower(x, y, 0.72, pick2 > 0.5 ? "#ff6090" : "#f0d040", "#ffffff");
      }
    });

    // Small flower & bush accent clusters in meadow biomes
    if (originalBiome === 0 || originalBiome === 6 || originalBiome === 7 || originalBiome === 5) {
      const fcount = 4;
  ...
  function drawRegion(r, idx, pass, isIslet) {
    ...
    if (pass === 0) {
      // 1. Static base shallow water outline (foam rim) - kept static to maximize performance
      const shallow = getOrganicPath(r.x, r.y, rx + 4, ry + 3.5, seed * 1.7, `${cachePrefix}_shallow_base`);
      fillPath(shallow, "rgba(56, 189, 248, 0.75)"); // bright sky blue foam
      return;
    }
    ...
    // 1. Draw textures (clipped inside territory)
    ctx.save();
    ctx.clip();
    ...
    if (conflict) {
      ...
      ctx.save();
      // Pulsing emerald-gold construction aura fill for pioneering territory
      ctx.globalAlpha = 0.30 + Math.sin(state.tick * 5) * 0.08;
      fillPath(land, "#10b981");
      ctx.restore();
    } else if (rel === "own") {
      ctx.save();
      // Soft pulsing cyan royal empire aura fill for user's territory
      ctx.globalAlpha = 0.24 + Math.sin(state.tick * 4) * 0.05;
      fillPath(land, "#0284c7");
      ctx.restore();
    } else if (rel === "ally") {
      ctx.save();
      // Subtle gold-yellow aura fill for alliance members
      ctx.globalAlpha = 0.14;
      fillPath(land, "#eab308");
      ctx.restore();
    } else if (rel === "enemy" && ownerCode > 1) {
      ctx.save();
      // Subtle crimson red aura fill for non-alliance / enemy players
    ...
    if (state.selectedRegion === idx) {
      ctx.save();
      // Draw bright transparent gold highlight matching exactly the land path shape
    ...
    // 2. Electric Cyan Bold Glowing Border ONLY for Player-owned Territory!
    else if (rel === "own") {
      ctx.save();
      ctx.beginPath();
      ctx.moveTo(land[0][0], land[0][1]);
      ...
      // Outer Glowing Neon Cyan Aura
      ctx.shadowColor = "#00f0ff";
      ctx.shadowBlur = 28;
      ctx.globalAlpha = 0.95 + Math.sin(state.tick * 4) * 0.05;
      ctx.strokeStyle = "#00f0ff";
      ctx.lineWidth = 6.5;
      ctx.stroke();

      // Inner Animated Dashed White Royal Border (moves continuously!)
      ctx.shadowBlur = 0;
      ctx.globalAlpha = 0.95;
      ctx.strokeStyle = "#ffffff";
      ctx.lineWidth = 2.5;
      ctx.setLineDash([12, 6]);
      ctx.lineDashOffset = -state.tick * 20;
      ctx.stroke();
      ctx.restore();
    }
    // 3. Subtle Soft Gold-Yellow Border for Alliance Members!
    else if (rel === "ally") {
      ctx.save();
      ctx.beginPath();
      ctx.moveTo(land[0][0], land[0][1]);
      ...
      // Subtle, clean 2px Gold Border without heavy glowing shadow
      ctx.shadowBlur = 0;
    ...
    // 4. Subtle Soft Crimson Red Border for Non-Alliance / Enemy Players!
    else if (rel === "enemy" && ownerCode > 1) {
      ctx.save();
      ctx.beginPath();
      ctx.moveTo(land[0][0], land[0][1]);
      ...
      // Subtle, clean 1.8px Crimson Red Border without heavy glowing shadow
      ctx.shadowBlur = 0;
      ctx.strokeStyle = "#ef4444";
      ctx.lineWidth = 1.8;
    ...
    if (state.selectedRegion === idx) {
      ...
      ctx.shadowBlur = 32;
      ctx.globalAlpha = 0.9 + Math.sin(state.tick * 6) * 0.1;
      ...
      ctx.save();
      const radius = 24 + Math.sin(state.tick * 6) * 4;
      ctx.shadowColor = "#ffe85a";
      ctx.shadowBlur = 12;
      
      // Pulsing outer thin ring
      ctx.beginPath();
      ...
      ctx.beginPath();
      // Left tick
      ctx.moveTo(r.x - radius - 6, r.y);
      ctx.lineTo(r.x - radius + 2, r.y);
      // Right tick
      ctx.moveTo(r.x + radius - 2, r.y);
      ctx.lineTo(r.x + radius + 6, r.y);
      // Top tick
      ctx.moveTo(r.x, r.y - radius - 6);
      ctx.lineTo(r.x, r.y - radius + 2);
      // Bottom tick
      ctx.moveTo(r.x, r.y + radius - 2);
      ctx.lineTo(r.x, r.y + radius + 6);
      ctx.strokeStyle = "#ffd34d";
      ctx.lineWidth = 2.0;
      ctx.stroke();
  ...
  function drawOakTree(x: number, y: number, scale?: number) {
    const sc = scale || 1;
    pxRect(x - 8 * sc, y + 8 * sc, 16 * sc, 4 * sc, "rgba(0,0,0,0.22)");
    pxRect(x - 2 * sc, y + 2 * sc, 4 * sc, 8 * sc, "#4a2a18");
    pxRect(x - 1 * sc, y + 4 * sc, 2 * sc, 5 * sc, "#6a3e23");
    pxRect(x - 9 * sc, y - 2 * sc, 18 * sc, 8 * sc, "#18401d");
    pxRect(x - 8 * sc, y - 7 * sc, 16 * sc, 7 * sc, "#22592a");
    pxRect(x - 6 * sc, y - 13 * sc, 12 * sc, 8 * sc, "#2e7a3a");
    pxRect(x - 3 * sc, y - 17 * sc, 7 * sc, 6 * sc, "#44a350");
    pxRect(x - 4 * sc, y - 9 * sc, 3 * sc, 3 * sc, "#7be88b");
  }
  ...
  function drawMountain(x, y, scale) {
    scale = scale || 1;
    pxRect(x - 26 * scale, y + 17 * scale, 52 * scale, 8 * scale, "rgba(0,0,0,0.28)");
    ...
    // Central Ridge line
    pxRect(x - 1 * scale, y - 28 * scale, 2 * scale, 46 * scale, "#9bb0c1");
    ...
    // Dark rocks base border
    ctx.strokeStyle = "#1b2126";
    ctx.lineWidth = 2 * scale;
    ctx.strokeRect(x - 22 * scale, y + 17 * scale, 44 * scale, 2 * scale);
  }
  ...
  function settlerOriginForRegion(regionId: number) {
    ...
    if (nearestTown && !state.newbieMode) {
  ...
  function drawWorld() {
    ...
    for (let i = 0; i < islets.length; i++) {
      const r = islets[i];
      if (isRegionInViewport(r, vp)) visibleIslets.push([r, i + 1000]);
    }
    for (let i = 0; i < regions.length; i++) {
      const r = regions[i];
      if (isRegionInViewport(r, vp)) visibleRegions.push([r, i]);
    }
  ...
  function drawResourceIcon(type, x, y) {
    const s = 1.65; // 1.65x size scale factor for even larger and clearer resource icons on territories
    if (type === "gold") drawCoin(x, y, 1.5);
    if (type === "wood") {
      if (y < 50) {
    ...
    if (type === "stone") {
      if (y < 50) {
        ...
        drawRockPile(x, y, 1.25);
      }
    }
    if (type === "gems") {
      if (y < 50) {
        // Top Bar HUD compact Gem icon
        pxRect(x - 8, y - 10, 16, 14, "#06b6d4");
        pxRect(x - 4, y - 14, 8, 8, "#a5f3fc");
      } else {
        // World Map: Replace cyan block shapes with beautiful natural rock piles!
        drawRockPile(x, y, 1.1);
      }
    }
  }
  ...
  function panToMinimapCoords(e) {
    ...
    // Map to world coordinates (0 to 12000, 0 to 9000)
    const worldX = Math.max(0, Math.min(12000, mx * 75));
    const worldY = Math.max(0, Math.min(9000, my * 75));
  ...
  function drawMinimap() {
    ...
    regions.forEach((r) => {
      ...
      const mx = r.x / 75;
      const my = r.y / 75;
      const mrx = (r.rx || r.r) / 75 * 1.02;
      const mry = (r.ry || r.r * 0.78) / 75 * 1.02;
    ...
    islets.forEach((r) => {
      ...
      const mx = r.x / 75;
      const my = r.y / 75;
      const mrx = (r.rx || r.r) / 75 * 0.82;
      const mry = (r.ry || r.r * 0.78) / 75 * 0.82;
      minimapCtx.ellipse(mx, my, mrx, mry, 0, 0, TAU);
      minimapCtx.fill();
    });
  ...
  function regionAtCoords(x: number, y: number): number {
    ...
    regions.forEach((r, i) => {
      ...
      if (nx * nx + ny * ny <= 1) {
        ...
        if (!bestRegion || d < bestRegion.d) bestRegion = { id: i, d };
      }
    });
    ...
    islets.forEach((r, i) => {
      ...
      if (nx * nx + ny * ny <= 1) {
        const d = Math.hypot(x - r.x, y - r.y);
        if (!bestIslet || d < bestIslet.d) bestIslet = { id: i + 1000, d };
      }
    });
  ...
  function getOptimalTownCenter(regionId: number): { x: number; y: number } {
    ...
    const isPositionSafe = (px: number, py: number) => {
      ...
      for (let i = 0; i < otherLands.length; i++) {
        const otherId = i >= regions.length ? (i - regions.length + 1000) : i;
    ...
    Array.from(new Set(regionIds || [])).forEach((regionId) => {
      ...
      towns.forEach((town) => {
        ...
        if (ownerCode === 1) {
          ...
          town.population = Math.max(town.population || 0, 32);
  ...
  function ensureTownForRegion(regionId, ownerCode) {
    ...
    if (town) {
      ...
      town.population = Math.max(town.population || 0, ownerCode === 1 ? 32 : 64);
      return town;
    }
    const pt = getOptimalTownCenter(regionId);
    town = {
  ...
  function handleButton(id) {
    ...
    if (id === "trainInfantry") {
      ...
      if (spend({ gold: gameConfig.infantryCostGold, wood: gameConfig.infantryCostWood, ...UNIT_EXTRA_COSTS.infantry })) {
    ...
    if (id === "trainCavalry") {
      ...
      if (spend({ gold: gameConfig.cavalryCostGold, wood: gameConfig.cavalryCostWood, stone: gameConfig.cavalryCostStone, ...UNIT_EXTRA_COSTS.cavalry })) {
        t.population = Math.max(0, t.population - popCost);
    ...
    if (id === "trainArtillery") {
      ...
      if (spend({ gold: gameConfig.artilleryCostGold, stone: gameConfig.artilleryCostStone, ...UNIT_EXTRA_COSTS.artillery })) {
        t.population = Math.max(0, t.population - popCost);
  ...
  function resolveBattleFinal(b) {
    ...
    if (b.originalOwner !== 0) {
      ...
      if (attWon) {
        const lostRegionId = regionAtCoords(target.x, target.y);
  ...
  function launchVoyage(source, target, power, targetRegionId, isAttack, infantry = 0, cavalry = 0, artillery = 0, battleSide: "attacker" | "defender" | null = null, backendTiming: any = null) {
    ...
    const localDuration = Math.max(6, Math.round((distanceKm / speed) * GAME_HOUR_SECONDS));
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    if (state.drag) {
      ...
      if (totalDist > 12) state.dragMoved = true;
      clampPan();
      return;
    }
  ...
  canvas.addEventListener("click", (e) => {
    const p = pointer(e);
    const totalDist = Math.hypot(p.x - dragStartPos.x, p.y - dragStartPos.y);
    if (state.dragMoved || totalDist > 12) {
      state.dragMoved = false;
      return;
    }
  ...
  function sim(dt) {
    ...
    if (state.settlerTravel?.returning) {
      state.settlerTravel.returnProgress = Math.min(1, (state.settlerTravel.returnProgress || 0) + dt / 12);
    ...
    const allTerritories: { r: any; idx: number }[] = [
      ...regions.map((r, i) => ({ r, idx: i })),
      ...islets.map((r, i) => ({ r, idx: i + 1000 })),
    ];
    allTerritories.forEach(({ r, idx }) => {
      if (state.regionOwnership[idx] !== 1) return;
      const biome = r.biome ?? 0;
      const y = BIOME_YIELDS[biome] || BIOME_YIELDS[0];
  ...
  return {
    ...
    if (id === "claimRegion") {
      ...
      if (id === "centerCamera") {
        state.zoom = FIXED_FAR_ZOOM;
        state.targetZoom = FIXED_FAR_ZOOM;
        state.panX = W / 2 - payload.x * state.zoom - (1 - state.zoom) * W * 0.48;
        state.panY = H / 2 - payload.y * state.zoom - (1 - state.zoom) * H * 0.48;
        state.selected = null;
        state.selectedRegion = null;
        clampPan();
        toast(`ỐNG NHÒM: CAMERA ĐÃ PHÓNG ĐẾN THÀNH PHỐ ${payload.townId}`);
        saveCamera();
        save();
        return;
      }
