...
export function AllyModal({
  ...
  const runAction = async (
    action: () => Promise<{ alliance: AllianceInfo | null }>,
    success: string,
  ) => {
    if (!token) return;
    setError(null);
  ...
  const resourceText = (resources: Record<string, number | undefined>) => {
    ...
    const parts = Object.entries(resources)
      ...
      .map(
        ([key, value]) => `${labels[key] ?? key} +${Math.floor(value || 0)}`,
      );
  ...
  return (
    <div className="modal-overlay alliance-overlay">
      <div className="alliance-modal">
        <button
          type="button"
          className="alliance-close"
          onClick={onClose}
          aria-label="Đóng"
        >
          <AssetIcon asset="close" size={21} />
        </button>
        <div className="alliance-title">LIÊN MINH VƯƠNG QUỐC</div>
        <div className="alliance-subtitle">
          Tạo liên minh tốn 100 kim cương. Mỗi liên minh tối đa 20 thành viên,
          có thể viện trợ tài nguyên và quân dự bị.
        </div>
        ...
        {loading ? (
          ...
          <div className="alliance-grid">
            <section className="alliance-panel alliance-main-panel">
              ...
              <div className="alliance-stats">
                ...
                <div>
                  <span>Vai trò</span>
                  <strong>
                    {alliance.leaderId === playerId ? "Minh chủ" : "Thành viên"}
                  </strong>
                </div>
                <div>
                  <span>Viện trợ chờ nhận</span>
                  <strong>{aidInbox.length}</strong>
                </div>
                <div>
                  <span>Quân dự bị LM</span>
                  <strong>{allianceTroopReserve}</strong>
                </div>
              </div>
              <button
                type="button"
                ...
                onClick={() =>
                  token &&
                  runAction(() => leaveAlliance(token), "Đã rời liên minh")
                }
              >
                RỜI LIÊN MINH
              </button>
              ...
              <div className="alliance-member-list">
                {alliance.members.map((member) => (
                  <div key={member.playerId} className="alliance-member-row">
                    ...
                    <strong>
                      {member.role === "leader" ? "MINH CHỦ" : "THÀNH VIÊN"}
                    </strong>
                  </div>
                ))}
              </div>
            ...
            <section className="alliance-panel">
              ...
              <div className="alliance-aid-grid">
                {(["gold", "wood", "stone", "food", "troops"] as const).map(
                  (key) => (
                    <label key={key}>
                      <span>
                        {key === "gold"
              ...
              <div className="alliance-note">
                Gửi lính tốn thêm 1 vàng + 2 lúa mỗi lính, tối đa 500 lính/lần.
              </div>
              <button
                type="button"
              ...
              <div className="alliance-list">
                {aidInbox.length > 0 ? (
                  aidInbox.map((aid) => (
                    <div
                      key={aid.id}
                      className="alliance-list-row alliance-aid-row"
                    >
                      <div>
                        <strong>{aid.fromName}</strong>
                        <span>
                          {resourceText(aid.resources)}
                          {aid.troops > 0 ? ` · Lính +${aid.troops}` : ""}
                        </span>
                      </div>
                      <button
                        type="button"
                        onClick={() =>
                          runStateAction(
                            () => claimAllianceAid(token!, aid.id),
                            "Đã nhận viện trợ liên minh",
                          )
                        }
                      >
                        NHẬN
                  ...
                  <div className="alliance-empty">
                    Chưa có viện trợ chờ nhận.
                  </div>
                )}
              </div>
            </section>
          ...
          <div className="alliance-grid">
            <section className="alliance-panel">
              ...
              <div className="alliance-cost">
                Chi phí lập liên minh: 100 kim cương
              </div>
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Tên liên minh"
                maxLength={32}
              />
              <input
                value={tag}
                onChange={(e) => setTag(e.target.value.toUpperCase())}
                placeholder="TAG 2-6 ký tự"
                maxLength={6}
              />
              <div className="alliance-emblem-picker">
                {(["shield", "star", "tower", "anchor", "flame"] as const).map(
                  (item) => (
                    <button
                      key={item}
                      type="button"
              ...
              <button
                ...
                onClick={() =>
                  runAction(
                    () => createAlliance(token!, name, tag, emblem),
                    "Đã tạo liên minh",
                  )
                }
              >
            ...
            <section className="alliance-panel">
              ...
              <div className="alliance-list">
                {alliances.length > 0 ? (
                  alliances.map((item) => (
                    <div key={item.id} className="alliance-list-row">
                      <div>
                        ...
                        <span>
                          {emblemLabel(item.emblem)} · {item.memberCount}/
                          {item.maxMembers} thành viên
                        </span>
                      </div>
                      <button
                        type="button"
                        onClick={() =>
                          runAction(
                            () => joinAlliance(token!, item.id),
                            `Đã gia nhập ${item.name}`,
                          )
                        }
                      >
                        GIA NHẬP
                  ...
                  <div className="alliance-empty">
                    Chưa có liên minh nào. Hãy lập liên minh đầu tiên.
                  </div>
                )}
              </div>
            </section>
