...
function EmblemIcon({ id, className = "" }: { id: string; className?: string }) {
  ...
  return <svg viewBox="0 0 48 48" className={className} aria-hidden="true">
    ...
    {id === "star" && <><path {...line} d="M24 5l5.2 11.1 12.2 1.8-8.8 8.6 2.1 12.2L24 33l-10.7 5.7 2.1-12.2-8.8-8.6 12.2-1.8L24 5z"/><circle cx="24" cy="24" r="2" fill="currentColor"/></>}
    {id === "tree" && <><path {...line} d="M24 6v35M18 41h12"/><path {...line} d="M24 8L13 21h6L9 31h11l-5 7h18l-5-7h11L29 21h6L24 8z"/></>}
  </svg>;
}
...
export function KingdomCreationModal({ onClose, onConfirm, defaultCityName = "Vương Quốc Tân Lập", territoryName = "Chưa chọn lãnh thổ", checkName, required = false }: KingdomCreationModalProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (stage === 1) { if (nameStatus === "available") setStage(3); return; }
  ...
  const preview = <div className={`founding-preview-card ${stage === 3 ? "complete" : ""}`}>
    ...
    <div className="founding-preview-info"><EmblemIcon id={selectedEmblem}/><div><small>VƯƠNG QUỐC</small><strong>{cityName.trim() || "Chưa đặt tên"}</strong><span>{territoryName} · {selectedColorInfo.name}</span></div></div>
  </div>;
  ...
  return createPortal(<div className="founding-backdrop" role="dialog" aria-modal="true" aria-label="Thành lập Vương Quốc">
    <section className="founding-card">
      ...
      <div className="founding-progress"><div className={stage === 1 ? "active" : "done"}><span>01</span><b>Danh xưng & Quốc gia</b></div><i/><div className={stage === 2 ? "active" : stage === 3 ? "done" : ""}><span>02</span><b>Sắc cờ</b></div><i/><div className={stage === 3 ? "active" : ""}><span>03</span><b>Hoàn thành</b></div></div>
      <form className="founding-body" onSubmit={submit}>
        {preview}
        <div className="founding-panel">{stage === 1 ? <>
          ...
          <div className="founding-section founding-country-section"><label><span>02</span> Chọn Quốc Gia <b>{selectedArchitectureInfo.name}</b></label><p className="founding-country-hint">Biểu tượng quốc gia quyết định kiến trúc Hoàng Thành, Quân Khu và trụ cờ trên bản đồ.</p><div className="founding-emblems founding-architectures">{KINGDOM_ARCHITECTURES.map((architecture) => <button key={architecture.id} type="button" aria-label={`Chọn quốc gia ${architecture.name}`} aria-pressed={selectedArchitecture === architecture.id} className={selectedArchitecture === architecture.id ? "selected" : ""} onClick={() => setSelectedArchitecture(architecture.id)}><span className={`founding-nation-emblem founding-nation-emblem--${architecture.id}`} aria-hidden="true"/><small>{architecture.name}</small></button>)}</div></div>
          <div className="founding-section founding-territory-color-section"><label><span>03</span> Màu Lãnh Thổ <b>{selectedColorInfo.name}</b></label><p className="founding-country-hint">Màu này đánh dấu toàn bộ lãnh thổ và quân đội thuộc vương quốc của bạn.</p><div className="founding-colors">{FLAG_COLORS.map((color) => <button key={color.id} type="button" aria-label={`Chọn ${color.name}`} aria-pressed={selectedColor === color.id} className={selectedColor === color.id ? "selected" : ""} onClick={() => setSelectedColor(color.id)} style={{ "--color": color.id } as CSSProperties}><i/></button>)}</div></div>
        </> : stage === 2 ? <>
        ...
        <footer className="founding-actions"><p>{stage === 1 ? "Đặt danh xưng, chọn quốc gia và màu lãnh thổ cho vương quốc của bạn." : stage === 2 ? "Chọn màu hiển thị lãnh thổ của vương quốc." : "Sau khi hoàn tất, hãy chọn lãnh thổ phù hợp để dựng Hoàng Thành."}</p><div>{stage > 1 && <button type="button" className="secondary" onClick={() => setStage(1)} disabled={submitting}>QUAY LẠI</button>}<button type="submit" className="primary" disabled={submitting || (stage === 1 && nameStatus !== "available")}>{submitting ? "ĐANG THÀNH LẬP" : stage === 1 ? "XEM XÁC NHẬN" : stage === 2 ? "XEM XÁC NHẬN" : "HOÀN TẤT THÀNH LẬP"}</button></div></footer>
      </form>
    </section>
  </div>, document.body);
}
