...
import { connectedPlayerIds, publishRealtime, realtimeStats } from "../realtime/socket.js";
...
const ShopEquipSchema = z.object({
...
const RESOURCE_KEYS = ["gold", "wood", "stone", "food", "iron", "coal", "sulfur", "gems"];
...
function verifyAntiBotProof(req, proof) {
    ...
    if (!challenge.nonce || challenge.ip !== antiBotIp(req) || !Number.isFinite(challenge.expiresAt) || challenge.expiresAt <= Date.now())
        return false;
    if (usedAntiBotChallenges.has(proof.challengeToken))
        return false;
    ...
    if (usedAntiBotChallenges.size > 10_000) {
        const now = Date.now();
        usedAntiBotChallenges.forEach((expiresAt, token) => { if (expiresAt <= now)
            usedAntiBotChallenges.delete(token); });
    }
    return true;
}
 ...
 */
function calcClearingSeconds(rx, ry, biome, settlerSpeed = 35, gameHourSeconds = 30) {
    const area = rx * ry;
...
function calcSpecialResources(t) {
    ...
    if ((t.biome === 2 || t.biome === 3 || t.biome === 4 || t.biome === 6) && t.id % 2 === 0)
        specials.push("Mỏ sắt");
    if ((t.biome === 1 || t.biome === 2 || t.biome === 3 || t.biome === 6) && area >= 14000)
        specials.push("Mỏ đá");
...
function getStaticTerritory(id) {
    if (!staticTerritoryByIdCache) {
        staticTerritoryByIdCache = new Map(buildStaticTerritoryList().map((territory: any) => [territory.id, territory]));
    }
...
function territoryConnectionType(source, target) {
    ...
    if (!source.isIslet && !target.isIslet && normDistSq <= 0.85)
        return "land";
    const targetIsCoastal = Boolean(target.isIslet || target.coastal || target.specialResources?.includes("Bến tàu tự nhiên"));
    ...
    const sourceHasHarbor = Boolean(source.isIslet || source.specialResources?.includes("Bến tàu tự nhiên"));
    if (sourceHasHarbor && targetIsCoastal)
        return "sea";
    return null;
}
...
async function pruneDisconnectedClaims(playerId) {
    ...
    while (queue.length > 0) {
        ...
        const children = (claims as any[]).filter((c) => !connectedIds.has(c.territoryId) && c.parentTerritoryId === currentId);
        for (const child of children) {
            connectedIds.add(child.territoryId);
            queue.push(child.territoryId);
        }
    }
    const disconnectedClaims = (claims as any[]).filter((c) => !connectedIds.has(c.territoryId));
    if (disconnectedClaims.length === 0) {
        await territoryClaims.updateMany({ playerId, isolated: true }, { $set: { isolated: false } });
        return [];
    }
    const byId = new Map(claims.map((claim: any) => [claim.territoryId, claim]) as any);
    const hasSeaLineage = (claim: any) => {
        ...
        let current = claim;
        while (current && !visited.has((current as any).territoryId)) {
            if (current.connectionType === "sea")
                return true;
            visited.add((current as any).territoryId);
            current = current.parentTerritoryId === undefined ? undefined : byId.get(current.parentTerritoryId);
        }
        return false;
    };
    const isolatedClaims = disconnectedClaims.filter(hasSeaLineage) as any[];
    const destroyedClaims = disconnectedClaims.filter((claim: any) => !hasSeaLineage(claim)) as any[];
    ...
    await Promise.all([
        isolatedIds.length > 0
            ? territoryClaims.updateMany({ playerId, territoryId: { $in: isolatedIds } }, { $set: { isolated: true } })
            : Promise.resolve(),
        territoryClaims.updateMany({ playerId, territoryId: { $in: [...connectedIds] as number[] } }, { $set: { isolated: false } }),
    ]);
    if (disconnectedIds.length === 0) {
        await bumpWorldCacheVersion();
        publishRealtime({ type: "world_state_hint", reason: "server_resync" }, `player:${playerId}`);
    ...
    if (saveDoc && Array.isArray(saveDoc.towns)) {
        const remainingTowns = saveDoc.towns.filter((town) => !disconnectedIds.includes(normalizeWorldTerritoryId(town.territoryId ?? town.id)));
...
function defaultTownSnapshotForTerritory(territory: any, playerId: any, townId?: any) {
    const population = territoryStartingPopulationForTown(territory, 1);
    const gameConfig = cachedGameConfig || DEFAULT_CONFIG;
    const troopCapacity = Math.max(1, Math.floor(population * gameConfig.strongholdTroopCapacityMultiplier));
    const now = new Date();
    return {
        ...
        nextTroopRecoveryAt: new Date(now.getTime() + gameConfig.troopRecoverySeconds * 1000).toISOString(),
        troopRecoverySeconds: gameConfig.troopRecoverySeconds,
        troopRecoveryBlockedReason: null,
    };
}
function normalizeTownSnapshotForState(town: any, playerId: any, territory?: any, now: any = new Date()) {
    const gameConfig = cachedGameConfig || DEFAULT_CONFIG;
    ...
    const infantryCount = Math.max(0, Math.floor(Number(town?.infantryCount ?? town?.troops ?? 0) || 0));
    const cavalryCount = Math.max(0, Math.floor(Number(town?.cavalryCount ?? 0) || 0));
    const artilleryCount = Math.max(0, Math.floor(Number(town?.artilleryCount ?? 0) || 0));
    const unitCount = infantryCount + cavalryCount + artilleryCount;
    const capacityMultiplier = kind === "capital" || kind === "sub_capital"
        ? gameConfig.capitalTroopCapacityMultiplier
        : gameConfig.strongholdTroopCapacityMultiplier;
    const troopCapacity = Math.max(1, Math.floor(populationState.population * capacityMultiplier));
    const specialty = trainingSpecialtyForTerritory(territory, kind);
    const storedRecoveryAt = town?.nextTroopRecoveryAt ? new Date(town.nextTroopRecoveryAt) : null;
    const nextTroopRecoveryAt = storedRecoveryAt && Number.isFinite(storedRecoveryAt.getTime())
        ? storedRecoveryAt.toISOString()
        : new Date(now.getTime() + gameConfig.troopRecoverySeconds * 1000).toISOString();
    return {
        ...town,
...
function townSnapshotsForPlayer(saveTowns, territories, playerId, resources, now = new Date()) {
    ...
    territories
        ...
        const existingIndex = towns.findIndex((town) => town?.id === townId ||
        ...
        if (existingIndex >= 0)
            towns[existingIndex] = normalized;
        else
...
function applyTownMarchReservations(towns, marches) {
    ...
    marches.forEach((march) => {
        ...
        const units = Math.max(0, Math.floor(Number(march?.infantry || 0)) +
            Math.floor(Number(march?.cavalry || 0)) +
            Math.floor(Number(march?.artillery || 0)));
        reservedByTerritory.set(territoryId, (reservedByTerritory.get(territoryId) || 0) + units);
    });
    towns.forEach((town) => {
        const territoryId = Number(town?.territoryId ?? normalizeWorldTerritoryId(town?.id));
        town.reservedTroops = reservedByTerritory.get(territoryId) || 0;
        town.troopCapacity = Math.max(1, Math.floor(Number(town?.troopCapacity ?? town?.maxTroops) || 1));
...
function advanceBattleHealth(battle, now = new Date()) {
    const startedAt = battle.startedAt instanceof Date ? battle.startedAt : new Date(battle.startedAt);
    const updatedAt = battle.hpUpdatedAt instanceof Date ? battle.hpUpdatedAt : battle.hpUpdatedAt ? new Date(battle.hpUpdatedAt) : startedAt;
    const durationSeconds = Math.max(1, Number(battle.durationSeconds) || 1);
    const elapsedSeconds = clampNumber((now.getTime() - updatedAt.getTime()) / 1000, 0, durationSeconds);
    const attackerMaxHp = Math.max(1, Number(battle.attackerMaxHp ?? battle.attackerPower) || 1);
    const defenderMaxHp = Math.max(1, Number(battle.defenderMaxHp ?? battle.defenderPower) || 1);
    const attackerLeads = Number(battle.attackerPower || 0) > Number(battle.defenderPower || 0);
    const attackerFloor = attackerLeads ? Math.max(1, Math.round(attackerMaxHp * 0.45)) : 0;
    const defenderFloor = attackerLeads ? 0 : Math.max(1, Math.round(defenderMaxHp * 0.45));
    const attackerDamagePerSecond = (attackerMaxHp - attackerFloor) / durationSeconds;
    const defenderDamagePerSecond = (defenderMaxHp - defenderFloor) / durationSeconds;
    const attackerCurrentHp = Math.max(attackerFloor, Math.min(attackerMaxHp, Math.round(Number(battle.attackerCurrentHp ?? attackerMaxHp) - attackerDamagePerSecond * elapsedSeconds)));
    const defenderCurrentHp = Math.max(defenderFloor, Math.min(defenderMaxHp, Math.round(Number(battle.defenderCurrentHp ?? defenderMaxHp) - defenderDamagePerSecond * elapsedSeconds)));
    return {
        attackerMaxHp,
        attackerCurrentHp,
        defenderMaxHp,
...
function toPublicBattle(battle) {
    ...
    const health = advanceBattleHealth(battle, battle.hpUpdatedAt instanceof Date ? battle.hpUpdatedAt : new Date(battle.hpUpdatedAt || Date.now()));
    return {
...
async function buildNationStatusSnapshot(playerId: any, resources: any, resourceCapacity: any, productionPerSecond: any, towns: any, context: any = {}, now: any = new Date()) {
    const { players, territoryClaims, marchOrders, territoryClearings, activeBattles } = await collections();
    const [player, claims, marches, clearings, battles] = await Promise.all([
        context.player !== undefined ? context.player : players.findOne({ _id: playerId }),
        context.claims !== undefined ? context.claims : territoryClaims.find({ playerId }).toArray(),
        context.marches !== undefined ? context.marches : marchOrders.find({ ownerId: playerId }).toArray(),
        context.clearings !== undefined ? context.clearings : territoryClearings.find({ playerId }).toArray(),
        context.battles !== undefined
            ? context.battles
            : activeBattles.find({ $or: [{ attackerId: playerId }, { defenderId: playerId }] }).toArray(),
    ]);
    ...
    const outboundTroops = marches.reduce((sum, march) => sum + Math.max(0, Number(march?.troops) || 0), 0);
    const garrisonTroops = towns.reduce((sum, town) => sum + Math.max(0, Number(town?.troops) || 0), 0);
    const ownedTroops = garrisonTroops + outboundTroops;
    const strategicPower = Math.round(ownedTroops * 1.2 +
        claims.length * 40 +
        normalizedResources.gold * 0.03 +
        normalizedResources.gems * 0.4);
    const attackedTerritoryIds = new Set(battles
        .filter((battle) => battle?.defenderId === playerId)
        .map((battle) => Number(battle?.regionId ?? battle?.territoryId))
        .filter(Number.isFinite));
    const buildingTerritoryIds = new Set(clearings.map((clearing) => Number(clearing?.territoryId)).filter(Number.isFinite));
    const claimByTerritoryId = new Map(claims.map((claim) => [Number(claim?.territoryId), claim]));
    const townStatuses = towns.map((town, index) => {
        const territoryId = Number(town?.territoryId ?? town?.regionId ?? town?.id);
        const claim = claimByTerritoryId.get(territoryId) as any;
        const kind = (town?.kind ?? claim?.settlementKind ?? (index === 0 ? "capital" : "military_district"));
        const storage = normalizeResources(town?.storage || {});
        const townCapacity = typeof town?.storageCapacity === "number"
            ? RESOURCE_KEYS.reduce((bag, key) => ({ ...bag, [key]: Number(town.storageCapacity) || 0 }), emptyResources())
            : normalizeResources(town?.storageCapacity || {});
        const storageUsed = RESOURCE_KEYS.reduce((sum, key) => sum + storage[key], 0);
        const storageCapacity = RESOURCE_KEYS.reduce((sum, key) => sum + townCapacity[key], 0);
        const population = Math.max(0, Math.floor(Number(town?.population) || 0));
        const populationCapacity = Math.max(population, Math.floor(Number(town?.populationCapacity) || population));
        ...
        return {
            ...
            maxTroops: Math.max(0, Math.floor(Number(town?.maxTroops) || populationCapacity * 10)),
            population,
            populationCapacity,
            storageUsed: Math.floor(storageUsed),
            storageCapacity: Math.floor(storageCapacity),
            storageUsagePercent: storageCapacity > 0 ? Math.min(100, Math.round(storageUsed / storageCapacity * 100)) : 0,
            status,
        };
    }).sort((a, b) => {
        const statusWeight = { under_attack: 0, building: 1, normal: 2 };
        const kindWeight = { capital: 0, sub_capital: 1, military_district: 2 };
        return statusWeight[a.status] - statusWeight[b.status] || kindWeight[a.kind] - kindWeight[b.kind] || a.townId - b.townId;
    });
    const population = townStatuses.reduce((sum, town) => sum + town.population, 0);
    const populationCapacity = townStatuses.reduce((sum, town) => sum + town.populationCapacity, 0);
    const underAttackCount = townStatuses.filter((town) => town.status === "under_attack").length;
    const storageNearFullCount = townStatuses.filter((town) => town.storageUsagePercent >= 90).length;
    const populationNearFullCount = townStatuses.filter((town) => town.populationCapacity > 0 && town.population / town.populationCapacity >= 0.95).length;
    const alerts = [];
    if (underAttackCount > 0)
        alerts.push({ code: "under_attack", count: underAttackCount, severity: "danger" });
    if (clearings.length > 0)
        alerts.push({ code: "building", count: clearings.length, severity: "info" });
    if (storageNearFullCount > 0)
        alerts.push({ code: "storage_near_full", count: storageNearFullCount, severity: "warning" });
    if (populationNearFullCount > 0)
        alerts.push({ code: "population_near_full", count: populationNearFullCount, severity: "warning" });
    const status = underAttackCount > 0
        ? "under_attack"
        : marches.length > 0
            ? "marching"
            : clearings.length > 0
...
async function publishPlayerState(playerId: any, reason: any, resources?: any, towns?: any, newbieShieldUntil?: any, context: any = {}) {
    const { territoryClaims, saves } = await collections();
    const claims = context.claims ?? await territoryClaims.find({ playerId }).toArray();
    const savedState = !resources || !Array.isArray(towns) ? await saves.findOne({ playerId }) : null;
    const effectiveResources = normalizeResources(resources || savedState?.resources || {});
    const effectiveTowns = Array.isArray(towns) ? towns : (Array.isArray(savedState?.towns) ? savedState.towns : []);
    ...
    const nationStatus = await buildNationStatusSnapshot(playerId, effectiveResources, resourceCapacity, productionPerSecond, effectiveTowns, { ...context, claims });
    ...
    const armyState = buildArmyState(playerId, {
        playerId,
        territories: [],
        clearings: [],
    ...
    publishRealtime({ type: "nation_state_updated", state: nationStatus, version: nowDate.getTime(), serverTime: now }, `player:${playerId}`);
    publishRealtime({ type: "army_state_updated", state: armyState, version: nowDate.getTime(), serverTime: now }, `player:${playerId}`);
}
function clampNumber(value, min, max) {
    return Math.max(min, Math.min(max, value));
}
function battleAttackPower(units, gameConfig) {
    return Math.max(0, Math.floor(Number(units.infantry || 0)) * gameConfig.infantryAttackPower +
...
function battleDefensePower(units, town, gameConfig) {
    const unitPower = Math.floor(Number(units.infantry || 0)) * gameConfig.infantryDefensePower +
...
function battleDurationSeconds(attackerPower, defenderPower, town, gameConfig) {
    const level = Math.max(1, Math.floor(Number(town?.level ?? town?.lvl ?? 1) || 1));
    const fort = Math.max(0, Math.floor(Number(town?.buildings?.fort || 0) || 0));
    const seconds = gameConfig.baseBattleSeconds +
...
function removeTownForTerritory(towns, territory) {
    const townId = townIdForTerritory(territory.id);
    return towns.filter((town) => town?.id !== townId &&
        Math.hypot((town?.x ?? 0) - territory.x, (town?.y ?? 0) - territory.y) >= 96);
}
async function buildWorldTerritoriesPayload() {
    ...
    claims.forEach((c) => {
        if (!claimsByPlayer.has(c.playerId))
            claimsByPlayer.set(c.playerId, []);
    ...
    const nameByOwner = new Map(ownerDocs.map((player: any) => [player._id, player.name]) as any);
    const flagColorByOwner = new Map(ownerDocs.map((player: any) => [player._id, player.flagColor]) as any);
    const emblemByOwner = new Map(ownerDocs.map((player: any) => [player._id, player.emblem]) as any);
...
function productionForClaims(claims: any) {
    ...
    const staticById = new Map(buildStaticTerritoryList().map((territory: any) => [territory.id, territory]));
...
function clearingBuildCostForRefund(clearing, territory, ownedCount) {
    if (clearing.buildCost)
        return clearing.buildCost;
    if (clearing.isStarterClaim || ownedCount === 0)
...
function maxDefendingTroopsForTown(town) {
    return Math.max(1, Math.floor(Number(town?.troopCapacity ?? town?.maxTroops) || Number(town?.population || 0) || 1));
}
...
async function collectPlayerResources(playerId, now = new Date()) {
    ...
    const elapsedSeconds = Math.max(0, Math.min(MAX_OFFLINE_RESOURCE_SECONDS, Math.floor((now.getTime() - lastCollectedAt.getTime()) / 1000)));
    ...
    RESOURCE_KEYS.forEach((key) => {
        ...
        next[key] = current[key] >= capacity[key]
    ...
    await players.updateOne({ _id: playerId }, {
        $set: {
            resources: next,
    ...
    const gainedEntries = RESOURCE_KEYS
    ...
    if (elapsedSeconds >= MIN_OFFLINE_REPORT_SECONDS && gainedEntries.length > 0) {
        ...
        const inserted = await playerMails.updateOne({ _id: mailDoc._id }, { $setOnInsert: mailDoc }, { upsert: true });
        if (inserted.upsertedCount > 0) {
            const unreadCount = await playerMails.countDocuments({ recipientId: playerId, readAt: null });
            const version = now.getTime();
            publishRealtime({ type: "mail_received", mail: toPublicMail(mailDoc), unreadCount, version, serverTime: now.toISOString() }, `player:${playerId}`);
        }
    }
...
async function buildGameStatePayload(playerId) {
    ...
    try {
        ...
        const towns = applyTownMarchReservations(townSnapshotsForPlayer(save?.towns, (world as any).territories, playerId, resourceState.resources), marches.filter((march) => march.ownerId === playerId));
        ...
        const playerClearings = clearings.filter((clearing) => clearing.playerId === playerId);
        const playerBattles = battles.filter((battle) => battle.attackerId === playerId || battle.defenderId === playerId);
        const nationStatus = await buildNationStatusSnapshot(playerId, resourceState.resources, resourceState.resourceCapacity, resourceState.productionPerSecond, towns, {
            player,
            claims: ownedClaims,
        ...
        if (towns.length > 0) {
            await saves.updateOne({ playerId }, {
...
function shopCatalog(gameConfig) {
    const testPrice = config.SHOP_TEST_MODE && config.NODE_ENV !== "production" ? 1 : null;
    const packAmount = Math.max(100, Math.min(1500, Math.floor(gameConfig.shopResourcePackAmount || 500)));
    return [
        {
            id: "pack_basic_all",
            type: "resource_pack",
            ...
            priceGems: testPrice ?? gameConfig.shopResourcePackPriceGems,
            testPrice: testPrice !== null,
            resources: { food: packAmount, wood: packAmount, stone: packAmount, iron: packAmount, gold: Math.floor(packAmount * 0.8) },
        },
        {
            id: "pack_royal_all",
            type: "resource_pack",
            ...
            priceGems: testPrice ?? Math.floor(gameConfig.shopResourcePackPriceGems * 2.5),
            testPrice: testPrice !== null,
            resources: { food: packAmount * 2, wood: packAmount * 2, stone: packAmount * 2, iron: packAmount * 2, gold: Math.floor(packAmount * 1.6) },
        },
        {
...
function normalizeShopInventory(value) {
    return {
        ownedSkins: [...new Set(Array.isArray(value?.ownedSkins) ? value.ownedSkins.filter(Boolean) : [])],
...
function buildArmyState(playerId, gameState, version = Date.now()) {
    ...
    const marches = gameState.marches.filter((march) => march.ownerId === playerId);
    const battles = (gameState.battles || []).filter((battle) => battle.attackerId === playerId || battle.defenderId === playerId);
    const garrisonTroops = towns.reduce((sum, town) => sum + town.troops, 0);
    const outboundTroops = marches.reduce((sum, march) => sum + Math.max(0, march.troops || 0), 0);
    const infantry = towns.reduce((sum, town) => sum + town.infantry, 0) + marches.reduce((sum, march) => sum + Math.max(0, march.infantry || 0), 0);
    const cavalry = towns.reduce((sum, town) => sum + town.cavalry, 0) + marches.reduce((sum, march) => sum + Math.max(0, march.cavalry || 0), 0);
    const artillery = towns.reduce((sum, town) => sum + town.artillery, 0) + marches.reduce((sum, march) => sum + Math.max(0, march.artillery || 0), 0);
    return {
        playerId,
        garrisonTroops,
        outboundTroops,
    ...
    for (const march of arrived) {
        ...
        if (!targetClaim || !targetClaim.playerId) {
            if (march.kind === "attack") {
                ...
                await Promise.all([
                    territoryClaims.updateOne({ territoryId: territory.id }, {
                        $set: {
                            playerId: march.ownerId,
                            claimedAt: now,
                            settlementKind: "military",
                            parentTerritoryId: march.fromTerritoryId,
                            connectionType: march.usesShip ? "sea" : territoryConnectionType(getStaticTerritory(march.fromTerritoryId), territory) || "land",
                            isolated: false,
                        },
                        $setOnInsert: { _id: `territory:${territory.id}`, territoryId: territory.id },
                    }, { upsert: true }),
                    saves.updateOne({ playerId: march.ownerId }, { $set: { towns: attackerTowns, updatedAt: now }, $setOnInsert: { _id: `save:${march.ownerId}`, playerId: march.ownerId, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true }),
        ...
        if (targetClaim.playerId === march.ownerId) {
            ...
            const ownerTowns = Array.isArray(ownerSave?.towns) ? [...ownerSave.towns] : [];
            const targetTownIndex = ownerTowns.findIndex((town) => town?.id === townIdForTerritory(territory.id) ||
                Math.hypot((town?.x ?? 0) - territory.x, (town?.y ?? 0) - territory.y) < 96);
            const targetTown = normalizeTownSnapshotForState(targetTownIndex >= 0 ? ownerTowns[targetTownIndex] : defaultTownSnapshotForTerritory(territory, march.ownerId), march.ownerId, territory);
            targetTown.infantryCount += Math.max(0, Math.floor(Number(march.infantry || 0) || 0));
            targetTown.cavalryCount += Math.max(0, Math.floor(Number(march.cavalry || 0) || 0));
            targetTown.artilleryCount += Math.max(0, Math.floor(Number(march.artillery || 0) || 0));
            targetTown.troops = targetTown.infantryCount + targetTown.cavalryCount + targetTown.artilleryCount;
            if (targetTownIndex >= 0)
                ownerTowns[targetTownIndex] = targetTown;
            else
                ownerTowns.push(targetTown);
            await Promise.all([
                marchOrders.deleteOne({ _id: march._id }),
                saves.updateOne({ playerId: march.ownerId }, { $set: { towns: ownerTowns, updatedAt: now } }),
            ]);
            publishRealtime({ type: "march_removed", marchId: march._id, territoryId: territory.id, reason: "arrived_home" });
            await publishPlayerState(march.ownerId, "march_arrived_home", undefined, ownerTowns);
            continue;
        }
        ...
        if (existingBattle) {
            ...
            if (isAttackerSide) {
                ...
                existingBattle.attackerMaxHp = (existingBattle.attackerMaxHp || existingBattle.attackerPower - addedPower) + addedPower;
                existingBattle.attackerCurrentHp = Math.min(existingBattle.attackerMaxHp, (existingBattle.attackerCurrentHp || 0) + addedPower);
            }
            else {
                ...
                existingBattle.defenderMaxHp = (existingBattle.defenderMaxHp || existingBattle.defenderPower - addedPower) + addedPower;
                existingBattle.defenderCurrentHp = Math.min(existingBattle.defenderMaxHp, (existingBattle.defenderCurrentHp || 0) + addedPower);
            }
            existingBattle.battleVersion = Math.max(1, Number(existingBattle.battleVersion || 1)) + 1;
            await activeBattles.updateOne({ _id: existingBattle._id }, {
                $set: {
        ...
        const defenderSave = (await saves.findOne({ playerId: targetClaim.playerId }));
        const defenderTowns = Array.isArray(defenderSave?.towns) ? defenderSave.towns : [];
        const defenderTown = normalizeTownSnapshotForState(findTownForTerritory(defenderTowns, territory) || defaultTownSnapshotForTerritory(territory, targetClaim.playerId), targetClaim.playerId, territory);
...
async function processActiveBattles(now = new Date()) {
    ...
    for (const battle of resolved) {
        ...
        const releasePlayerLocks = await acquirePlayerMutationLocks([battle.attackerId, battle.defenderId]);
        try {
            ...
            const troopValue = (infantry, cavalry, artillery) => infantry * gameConfig.infantryTroopsValue + cavalry * gameConfig.cavalryTroopsValue + artillery * gameConfig.artilleryTroopsValue;
            ...
            if (attackerWins) {
                ...
                const attackerSave = (await saves.findOne({ playerId: battle.attackerId }));
                const defenderSave = battle.defenderId ? (await saves.findOne({ playerId: battle.defenderId })) : null;
                const attackerTowns = Array.isArray(attackerSave?.towns) ? removeTownForTerritory(attackerSave.towns, territory) : [];
                const defenderStoredTown = Array.isArray(defenderSave?.towns)
                    ? findTownForTerritory(defenderSave.towns, territory)
                    : null;
                const capturedStorage = normalizeStoredResources(defenderStoredTown?.storage);
                ...
                const attackerClaimCount = await territoryClaims.countDocuments({ playerId: battle.attackerId });
                const attackerCapacity = resourceCapacityForOwnedTerritories(attackerClaimCount + 1);
                RESOURCE_KEYS.forEach((key) => {
                    const stored = Math.min(defenderResources[key] || 0, capturedStorage[key] || 0);
                    const availableCapacity = Math.max(0, attackerCapacity[key] - (attackerResources[key] || 0));
                    lootedResources[key] = Math.floor(Math.min(stored, availableCapacity));
                ...
                await Promise.all([
                    territoryClaims.updateOne({ territoryId: territory.id }, {
                        $set: {
                            ...
                            connectionType: battle.usesShip ? "sea" : territoryConnectionType(getStaticTerritory(battle.fromTerritoryId), territory) || "land",
                            isolated: false,
                        },
                        $setOnInsert: { _id: `territory:${territory.id}`, territoryId: territory.id },
                    }, { upsert: true }),
                    territoryClearings.deleteMany({ territoryId: territory.id }),
                    saves.updateOne({ playerId: battle.attackerId }, { $set: { towns: attackerTowns, updatedAt: now }, $setOnInsert: { _id: `save:${battle.attackerId}`, playerId: battle.attackerId, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true }),
                ...
                const defenderSave = (await saves.findOne({ playerId: battle.defenderId }));
                const defenderTowns = Array.isArray(defenderSave?.towns) ? removeTownForTerritory(defenderSave.towns, territory) : [];
                ...
                await saves.updateOne({ playerId: battle.defenderId }, { $set: { towns: defenderTowns, updatedAt: now }, $setOnInsert: { _id: `save:${battle.defenderId}`, playerId: battle.defenderId, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true });
                ...
                if (retreatRatio > 0) {
                    ...
                    if (sourceTerritory) {
                        ...
                        if (retreatInfantry + retreatCavalry + retreatArtillery > 0) {
                            ...
                            const sourceTown = normalizeTownSnapshotForState(findTownForTerritory(attackerSave?.towns || [], sourceTerritory) || defaultTownSnapshotForTerritory(sourceTerritory, battle.attackerId), battle.attackerId, sourceTerritory);
                            ...
                            await saves.updateOne({ playerId: battle.attackerId }, { $set: { towns: attackerTowns, updatedAt: now }, $setOnInsert: { _id: `save:${battle.attackerId}`, playerId: battle.attackerId, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true });
            ...
            const reportRecipients = [...new Set([battle.attackerId, battle.defenderId].filter((id) => Boolean(id)))];
            await Promise.all(reportRecipients.map(async (recipientId) => {
...
async function cancelBrokenRouteClearing(clearing, now) {
    ...
    await Promise.all([
        ...
        saves.updateOne({ playerId: clearing.playerId }, { $set: { towns, updatedAt: now }, $setOnInsert: { _id: `save:${clearing.playerId}`, playerId: clearing.playerId, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true }),
    ]);
...
async function processCompletedClearings(now = new Date()) {
    ...
    for (const clearing of completed) {
        ...
        await territoryClaims.updateOne({ territoryId: territory.id }, {
            $setOnInsert: {
        ...
        towns.push(town);
        await saves.updateOne({ playerId: clearing.playerId }, {
            $set: { towns, updatedAt: now },
...
async function publishRealtimeEconomyTick(now) {
    ...
    const settlePlayer = async (playerId) => {
        ...
        try {
            ...
            const towns = townSnapshotsForPlayer(save?.towns, territories, playerId, resourceState.resources, now);
            if (towns.length > 0) {
                await saves.updateOne({ playerId }, {
            ...
            await publishPlayerState(playerId, "economy_tick", resourceState.resources, towns);
        }
        finally {
            releasePlayerLock();
        }
...
async function processDueTroopRecovery(now) {
    ...
    const gameConfig = await loadGameConfig();
    if (!gameConfig.troopRecoveryEnabled)
        return;
    const { saves, players, territoryClaims, marchOrders, activeBattles } = await collections();
    const dueSaves = await saves.find({
    ...
    for (const dueSave of dueSaves) {
        ...
        try {
            const [save, claims, marches, battles, resourceState] = await Promise.all([
                ...
                activeBattles.find({ $or: [{ attackerId: dueSave.playerId }, { defenderId: dueSave.playerId }] }).toArray(),
            ...
            const territories = claims
                .map((claim) => {
                const territory = getStaticTerritory(claim.territoryId);
                return territory ? { ...territory, ownerId: dueSave.playerId, settlementKind: claim.settlementKind } : null;
            })
                .filter((territory) => Boolean(territory));
            const towns = applyTownMarchReservations(townSnapshotsForPlayer(save.towns, territories, dueSave.playerId, undefined, now), marches);
            let resources = normalizeResources(resourceState.resources);
            let changed = false;
            const updates = [];
            const battleTerritories = new Set(battles.map((battle) => Number(battle.regionId)));
            const intervalMs = Math.max(10, gameConfig.troopRecoverySeconds) * 1000;
            for (const town of towns) {
                const territoryId = Number(town.territoryId ?? normalizeWorldTerritoryId(town.id));
                const dueAt = town.nextTroopRecoveryAt ? new Date(town.nextTroopRecoveryAt) : new Date(now.getTime() + intervalMs);
                ...
                const specialty = trainingSpecialtyForTerritory(territory, claim?.settlementKind || town.kind);
                const cost = troopRecoveryCost(specialty, gameConfig);
                const elapsedSlots = 1 + Math.floor(Math.max(0, now.getTime() - dueAt.getTime()) / intervalMs);
                const slots = Math.min(gameConfig.troopRecoveryOfflineLimit, elapsedSlots);
                let recovered = 0;
                let blockedReason = null;
                for (let slot = 0; slot < slots; slot += 1) {
                    const garrisonUnits = Math.max(0, Number(town.infantryCount || 0) + Number(town.cavalryCount || 0) + Number(town.artilleryCount || 0));
                    const usedCapacity = garrisonUnits + Math.max(0, Number(town.reservedTroops || 0));
                    const capacity = Math.max(1, Number(town.troopCapacity ?? town.maxTroops) || 1);
                    ...
                    if (specialty === "cavalry")
                        ...
                        town.artilleryCount = Math.max(0, Number(town.artilleryCount || 0)) + 1;
                    else
                        town.infantryCount = Math.max(0, Number(town.infantryCount || 0)) + 1;
                    town.troops = Number(town.infantryCount || 0) + Number(town.cavalryCount || 0) + Number(town.artilleryCount || 0);
                    recovered += 1;
                    changed = true;
                }
                ...
                town.nextTroopRecoveryAt = new Date(blockedReason ? now.getTime() + intervalMs : dueAt.getTime() + Math.max(1, recovered) * intervalMs).toISOString();
                updates.push({
                    townId: town.id,
                    territoryId,
                    specialty,
            ...
            await Promise.all([
                saves.updateOne({ playerId: dueSave.playerId }, { $set: { towns, resources, updatedAt: now } }),
                players.updateOne({ _id: dueSave.playerId }, { $set: { resources, lastResourceCollectedAt: now, lastSeenAt: now } }),
            ]);
            if (updates.length > 0) {
                publishRealtime({
            ...
            if (changed) {
                await publishPlayerState(dueSave.playerId, "troop_recovered", resources, towns);
            }
        }
        finally {
            releasePlayerLock();
...
async function publishActiveBattleStates(now) {
    ...
    if (now.getTime() - lastBattleStateBroadcastAt < intervalMs)
        return;
    lastBattleStateBroadcastAt = now.getTime();
    const { activeBattles } = await collections();
    const battles = await activeBattles.find({ resolvesAt: { $gt: now } }).limit(250).toArray();
    if (battles.length === 0)
        return;
    ...
    await activeBattles.bulkWrite(updatedBattles.map((battle) => ({
        updateOne: {
...
async function retireLegacyBots() {
    ...
    const bots = await players.find({ $or: [{ isBot: true }, { _id: /^player:bot-/ }] }).toArray();
...
async function ensureSeededBots() {
    ...
    for (let i = 0; i < BOT_CONFIGS.length; i++) {
        ...
        if (!existing) {
            ...
            const ALL_LANDS = Array.from({ length: 60 }, (_, idx) => getStaticTerritory(idx)).filter((t) => Boolean(t));
            ...
            if (staticT) {
                ...
                if (!claim) {
                    const botTown = defaultTownSnapshotForTerritory(staticT, cfg.id, 9000 + starterLandId) as any;
                    await Promise.all([
                        territoryClaims.insertOne({ _id: `territory:${starterLandId}`, territoryId: starterLandId, playerId: cfg.id, claimedAt: now }),
                        saves.updateOne({ playerId: cfg.id }, { $set: { towns: [botTown], updatedAt: now }, $setOnInsert: { _id: `save:${cfg.id}`, playerId: cfg.id, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true }),
                    ]);
                }
            }
        }
...
async function processBotAISimulation(now = new Date()) {
    try {
        ...
        const clearingByPlayer = new Map(activeBotClearings.map((clearing) => [clearing.playerId, clearing]));
        ...
        for (const bot of bots) {
            ...
            if (claims.length === 0) {
                ...
                if (unclaimed.length > 0) {
                    ...
                    const starterTown = defaultTownSnapshotForTerritory(pick, bot._id, 9000 + pick.id) as any;
                    await territoryClaims.insertOne({ _id: `territory:${pick.id}`, territoryId: pick.id, playerId: bot._id, claimedAt: now });
                    await saves.updateOne({ playerId: bot._id }, { $set: { towns: [starterTown], updatedAt: now }, $setOnInsert: { _id: `save:${bot._id}`, playerId: bot._id, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true });
                    allClaimedIds.add(pick.id);
                    const insertedClaim = { _id: `territory:${pick.id}`, territoryId: pick.id, playerId: bot._id, claimedAt: now };
                    allClaims.push(insertedClaim);
                    claimsByPlayer.set(bot._id, [insertedClaim]);
                    saveByPlayer.set(bot._id, { ...(botSave || {}), playerId: bot._id, towns: [starterTown] } as any);
                    worldChanged = true;
                    publishRealtime({
                        type: "territory_claimed",
                        territory: {
            ...
            if (!activeClearing && Math.random() < 0.40) {
                ...
                if (wildTargetId >= 0) {
                    ...
                    await territoryClearings.updateOne({ territoryId: wildTargetId }, {
                        $set: {
                    ...
                    clearingByPlayer.set(bot._id, { territoryId: wildTargetId, playerId: bot._id, startedAt: now, arrivesAt, completesAt } as any);
                    worldChanged = true;
                    publishRealtime({
            ...
            if (activeMarches < 2 && Math.random() < 0.30 && botTowns.length > 0) {
                const enemyClaims = allClaims.filter((claim) => claim.playerId !== bot._id);
                if (enemyClaims.length > 0) {
                    ...
                    if (sourceTown && (sourceTown.troops || 0) > 80) {
                        ...
                        const marchDoc: any = {
                            ...
                            ownerId: bot._id,
                            ownerName: bot.name,
                            fromTerritoryId: normalizeWorldTerritoryId((sourceTown as any).regionId ?? sourceTown.id ?? 0),
...
function calcTravelMetrics(from, to, units, marchConfig = DEFAULT_MARCH_CONFIG, forceShip = false) {
    ...
    if (usesShip) {
        ...
        if (units.infantry > 0)
            speeds.push(marchConfig.infantrySpeed);
        if (units.cavalry > 0)
            speeds.push(marchConfig.cavalrySpeed);
        if (units.artillery > 0)
            speeds.push(marchConfig.artillerySpeed);
    }
...
function territoryHasHarbor(territory) {
    return Boolean(territory.isIslet || territory.specialResources?.includes("Bến tàu tự nhiên"));
}
function territoryAcceptsLanding(territory) {
    return Boolean(territory.isIslet || territory.coastal || territory.specialResources?.includes("Bến tàu tự nhiên"));
}
function resolveAttackRoute(from, to) {
    ...
    if (hasPort && coastalTarget)
        return { valid: true, routeType: "sea" };
    if (!hasPort && coastalTarget)
        return { valid: false, routeType: null, reason: "Thành này không có Bến tàu tự nhiên" };
    if (hasPort && !coastalTarget)
        return { valid: false, routeType: null, reason: "Không thể đổ bộ thẳng vào lãnh thổ nội địa" };
    return { valid: false, routeType: null, reason: "Thành không giáp mục tiêu và không có tuyến biển hợp lệ" };
}
export function createApp() {
    ...
    app.get("/api/health", (_req, res) => {
        const payload = { ok: true, service: "island-empire-api", time: new Date().toISOString() };
        res.json(payload);
    });
    ...
    app.post("/api/auth/admin/login", async (req, res) => {
        ...
        if (!parsed.success)
            return res.status(400).json({ error: "bad_request", message: "Invalid login payload" });
    ...
    app.post("/api/auth/player/register", async (req, res) => {
        ...
        if (!enforceAuthAttempt(req, res, "register", username))
            return;
    ...
    app.post("/api/auth/player/login", async (req, res) => {
        ...
        if (!enforceAuthAttempt(req, res, "login", username))
            return;
    ...
    app.post("/api/auth/player/guest", async (req, res) => {
        ...
        if (!enforceAuthAttempt(req, res, "guest", name))
            return;
        const id = `guest:${name.toLowerCase().replace(/[^a-z0-9-]/g, "-")}`;
        const { players } = await collections();
        const now = new Date();
        await players.updateOne({ _id: id }, {
    ...
    app.get("/api/reports", requireAuth, async (req, res) => {
        try {
            ...
            res.json({ ok: true, reports: reports.map((report) => toPublicReport(report, playerId)), unreadCount });
        }
        catch (e) {
    ...
    app.get("/api/reports/:id", requireAuth, async (req, res) => {
        ...
        if (!report)
            return res.status(404).json({ error: "not_found", message: "Không tìm thấy chiến báo" });
    ...
    app.post("/api/reports/:id/read", requireAuth, async (req, res) => {
        ...
        const playerId = req.user!.id;
        const reportId = String(req.params.id);
        const result = await battleReports.updateOne({ _id: reportId, $or: [{ attackerId: playerId }, { defenderId: playerId }] }, { $addToSet: { readBy: playerId } });
        ...
        publishRealtime({ type: "battle_report_read", reportId, unreadCount, version, serverTime: new Date(version).toISOString() }, `player:${playerId}`);
        res.json({ ok: true, unreadCount });
    });
    app.post("/api/reports/read-all", requireAuth, async (req, res) => {
        const { battleReports } = await collections();
        const playerId = req.user!.id;
        await battleReports.updateMany({ $or: [{ attackerId: playerId }, { defenderId: playerId }], readBy: { $ne: playerId } }, { $addToSet: { readBy: playerId } });
        const version = Date.now();
        publishRealtime({ type: "battle_report_read", unreadCount: 0, version, serverTime: new Date(version).toISOString() }, `player:${playerId}`);
        res.json({ ok: true, unreadCount: 0 });
    });
    app.get("/api/mail/inbox", requireAuth, async (req, res) => {
        const { playerMails } = await collections();
        const mails = await playerMails.find({ recipientId: req.user!.id }).sort({ sentAt: -1 }).limit(80).toArray();
        const unreadCount = await playerMails.countDocuments({ recipientId: req.user!.id, readAt: null });
        res.json({ ok: true, mails: mails.map(toPublicMail), unreadCount });
    });
    app.get("/api/mail/sent", requireAuth, async (req, res) => {
        const { playerMails } = await collections();
        const mails = await playerMails.find({ senderId: req.user!.id }).sort({ sentAt: -1 }).limit(80).toArray();
        res.json({ ok: true, mails: mails.map(toPublicMail) });
    });
    app.post("/api/mail/send", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "mail:send", 8, 60_000))
            return;
        const parsed = SendMailSchema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: "bad_request", message: "Thư không hợp lệ" });
        const { players, playerMails } = await collections();
        const senderId = req.user!.id;
        const existing = await playerMails.findOne({ senderId, requestId: parsed.data.requestId });
        if (existing)
            return res.json({ ok: true, mail: toPublicMail(existing), duplicate: true });
        const [sender, recipient] = await Promise.all([
            players.findOne({ _id: senderId }),
            players.findOne({ $or: [{ _id: parsed.data.recipientId }, { name: parsed.data.recipientId }] }),
        ]);
        if (!recipient)
            return res.status(404).json({ error: "recipient_not_found", message: "Không tìm thấy người nhận" });
        if (recipient._id === senderId)
            return res.status(400).json({ error: "self_mail", message: "Không thể tự gửi thư cho mình" });
        ...
        const unreadCount = await playerMails.countDocuments({ recipientId: recipient._id, readAt: null });
        const version = sentAt.getTime();
        publishRealtime({ type: "mail_received", mail: toPublicMail(mailDoc), unreadCount, version, serverTime: sentAt.toISOString() }, `player:${recipient._id}`);
    ...
    app.post("/api/mail/:id/read", requireAuth, async (req, res) => {
        ...
        const result = await playerMails.updateOne({ _id: mailId, recipientId: playerId }, { $set: { readAt } });
        if (!result.matchedCount)
            return res.status(404).json({ error: "not_found", message: "Không tìm thấy thư" });
        const unreadCount = await playerMails.countDocuments({ recipientId: playerId, readAt: null });
        const version = readAt.getTime();
        publishRealtime({ type: "mail_read", mailId, unreadCount, version, serverTime: readAt.toISOString() }, `player:${playerId}`);
        res.json({ ok: true, unreadCount, readAt: readAt.toISOString() });
    });
    app.post("/api/player/profile", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "profile:update", 12, 60_000))
            return;
        ...
        if (flagColor)
            updateData.flagColor = flagColor;
        if (emblem)
    ...
    app.post("/api/player/active-map", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "player:active-map", 30, 60_000))
            return;
        const parsed = ActiveMapSchema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: "bad_request", message: "Bản đồ không hợp lệ" });
        const { players } = await collections();
        await players.updateOne({ _id: req.user!.id }, { $set: { activeMap: parsed.data.activeMap, lastSeenAt: new Date() } }, { upsert: true });
    ...
    app.get("/api/player/sync", requireAuth, stateReadLimiter, async (req, res) => {
        const playerId = req.user!.id;
        const gameState = await buildGameStatePayload(playerId);
        const { battleReports, playerMails, players } = await collections();
        const [reports, inbox, sent, reportUnreadCount, mailUnreadCount, player, gameConfig] = await Promise.all([
            battleReports.find({ $or: [{ attackerId: playerId }, { defenderId: playerId }] }).sort({ createdAt: -1 }).limit(50).toArray(),
            playerMails.find({ recipientId: playerId }).sort({ sentAt: -1 }).limit(80).toArray(),
            playerMails.find({ senderId: playerId }).sort({ sentAt: -1 }).limit(80).toArray(),
            battleReports.countDocuments({ $or: [{ attackerId: playerId }, { defenderId: playerId }], readBy: { $ne: playerId } }),
        ...
        const payload = {
            ...
            shopCatalog: shopCatalog(gameConfig),
            shopInventory: normalizeShopInventory(player?.shopInventory),
            version,
            serverTime: new Date(version).toISOString(),
        };
        res.json(payload);
    });
    app.get("/api/nation/state", requireAuth, stateReadLimiter, async (req, res) => {
        const gameState = await buildGameStatePayload(req.user!.id);
        res.json({ ok: true, state: gameState.nationStatus, version: Date.now(), serverTime: gameState.serverTime });
    });
    app.get("/api/army/state", requireAuth, stateReadLimiter, async (req, res) => {
        const version = Date.now();
        const gameState = await buildGameStatePayload(req.user!.id);
        res.json({ ok: true, state: buildArmyState(req.user!.id, gameState, version), version, serverTime: new Date(version).toISOString() });
    });
    app.get("/api/leaderboard/military", requireAuth, async (req, res) => {
        try {
            ...
            const leaderboardData = allSaves.map((save) => {
                const playerId = save.playerId;
                const player = playerMap.get(playerId) as any;
                if (!player)
                    return null;
                const towns = Array.isArray(save.towns) ? save.towns : [];
                const garrisonTroops = towns.reduce((sum, town) => sum + (Number(town.troops) || 0), 0);
            ...
            res.status(500).json({ error: "internal_server_error", message: "Lỗi tải bảng xếp hạng" });
        }
    });
    app.get("/api/shop/catalog", requireAuth, async (_req, res) => {
        res.json({ ok: true, products: shopCatalog(await loadGameConfig()), testMode: config.SHOP_TEST_MODE && config.NODE_ENV !== "production" });
    });
    app.get("/api/shop/inventory", requireAuth, async (req, res) => {
        const { players } = await collections();
        const player = await players.findOne({ _id: req.user!.id });
        res.json({ ok: true, inventory: normalizeShopInventory(player?.shopInventory) });
    });
    app.get("/api/shop/history", requireAuth, async (req, res) => {
        const { shopPurchases } = await collections();
        const purchases = await shopPurchases.find({ playerId: req.user!.id }).sort({ createdAt: -1 }).limit(80).toArray();
        res.json({
            ok: true,
    ...
    app.post("/api/shop/purchase", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "shop:purchase", 20, 60_000))
            return;
        const parsed = ShopPurchaseSchema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: "bad_request", message: "Yêu cầu mua không hợp lệ" });
        const playerId = req.user!.id;
        const release = await acquirePlayerMutationLock(playerId);
        try {
            const { players, saves, shopPurchases } = await collections();
            const existing = await shopPurchases.findOne({ playerId, requestId: parsed.data.requestId });
            if (existing) {
                const player = await players.findOne({ _id: playerId });
                const currentInventory = normalizeShopInventory(player?.shopInventory);
                const needsEquipRepair = Boolean(existing.grantedSkinId && parsed.data.equipTarget && (parsed.data.equipTarget === "capital"
                ...
                const repairedInventory: any = needsEquipRepair
                    ? normalizeShopInventory({
                        ...currentInventory,
                        equippedCapitalSkin: parsed.data.equipTarget === "capital" ? existing.grantedSkinId : currentInventory.equippedCapitalSkin,
                        equippedDistrictSkin: parsed.data.equipTarget === "military_district" ? existing.grantedSkinId : currentInventory.equippedDistrictSkin,
                        version: currentInventory.version + 1,
                    })
                    : currentInventory;
                if (repairedInventory.version !== currentInventory.version) {
                    await players.updateOne({ _id: playerId }, { $set: { shopInventory: repairedInventory } });
                }
                return res.json({
                    ok: true,
                    duplicate: true,
            ...
            const gameConfig = await loadGameConfig();
            const product = shopCatalog(gameConfig).find((item) => item.id === parsed.data.productId);
            if (!product)
                return res.status(404).json({ error: "product_not_found", message: "Sản phẩm không tồn tại" });
            const resourceState = await collectPlayerResources(playerId);
            const player = await players.findOne({ _id: playerId });
            const currentResources = normalizeResources(resourceState.resources);
            if (currentResources.gems < product.priceGems) {
                return res.status(400).json({ error: "insufficient_gems", message: "Không đủ ngọc để mua" });
            }
            ...
            if (product.skinId && inventory.ownedSkins.includes(product.skinId)) {
                return res.status(409).json({ error: "already_owned", message: "Bạn đã sở hữu ngoại trang này" });
            }
            const nextResources = { ...currentResources };
            if (product.resources) {
                for (const key of RESOURCE_KEYS) {
                    const grant = Math.max(0, Math.floor(Number(product.resources[key]) || 0));
                    if (grant > 0 && nextResources[key] + grant > resourceState.resourceCapacity[key]) {
                ...
                RESOURCE_KEYS.forEach((key) => {
                    nextResources[key] += Math.max(0, Math.floor(Number(product.resources?.[key]) || 0));
                });
            }
            nextResources.gems -= product.priceGems;
            const nextInventory: any = normalizeShopInventory({
                ...inventory,
                ownedSkins: product.skinId ? [...(inventory.ownedSkins as any[]), product.skinId] : inventory.ownedSkins,
                equippedCapitalSkin: product.skinId && parsed.data.equipTarget === "capital"
                    ? product.skinId
                    : inventory.equippedCapitalSkin,
                equippedDistrictSkin: product.skinId && parsed.data.equipTarget === "military_district"
                    ? product.skinId
            ...
            await players.updateOne({ _id: playerId }, { $set: { resources: nextResources, shopInventory: nextInventory, lastResourceCollectedAt: createdAt, lastSeenAt: createdAt } });
            await saves.updateOne({ playerId }, { $set: { resources: nextResources, updatedAt: createdAt } });
            ...
            res.json({ ok: true, purchase, inventory: nextInventory, resources: nextResources });
            void publishPlayerState(playerId, "shop_purchase", nextResources, null).catch((error) => {
    ...
    app.post("/api/shop/equip", requireAuth, async (req, res) => {
        ...
        if (!parsed.success)
            return res.status(400).json({ error: "bad_request", message: "Ngoại trang không hợp lệ" });
        const playerId = req.user!.id;
        const release = await acquirePlayerMutationLock(playerId);
        try {
            const { players } = await collections();
            ...
            if (!inventory.ownedSkins.includes(parsed.data.skinId)) {
                return res.status(403).json({ error: "not_owned", message: "Bạn chưa sở hữu ngoại trang này" });
            }
            const nextInventory: any = {
                ...inventory,
                equippedCapitalSkin: parsed.data.target === "capital" ? parsed.data.skinId : inventory.equippedCapitalSkin,
                equippedDistrictSkin: parsed.data.target === "military_district" ? parsed.data.skinId : inventory.equippedDistrictSkin,
                version: inventory.version + 1,
            };
            await players.updateOne({ _id: playerId }, { $set: { shopInventory: nextInventory } });
            const version = Date.now();
            publishRealtime({ type: "shop_inventory_updated", inventory: nextInventory, version, serverTime: new Date(version).toISOString() }, `player:${playerId}`);
    ...
    app.post("/api/alliance/create", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "alliance:create", 4, 60_000))
            return;
        ...
        try {
            ...
            await players.updateOne({ _id: req.user!.id }, { $set: { resources: { ...resourceState.resources, gems: resourceState.resources.gems - ALLIANCE_CREATE_GEMS_COST } } });
        }
        catch (err) {
            if (err?.code === 11000) {
        ...
        const payload = { ok: true, alliance: alliance ? await toPublicAlliance(alliance) : null };
        res.json(payload);
    });
    app.post("/api/alliance/join", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "alliance:join", 10, 60_000))
            return;
        ...
        await alliances.updateOne({ _id: alliance._id }, { $addToSet: { memberIds: req.user!.id }, $set: { updatedAt: new Date() } });
        await bumpWorldCacheVersion();
        const updated = await alliances.findOne({ _id: alliance._id });
        const payload = { ok: true, alliance: updated ? await toPublicAlliance(updated) : null };
        res.json(payload);
    });
    app.post("/api/alliance/leave", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "alliance:leave", 8, 60_000))
            return;
        const { alliances } = await collections();
        ...
        await alliances.updateOne({ _id: alliance._id }, { $pull: { memberIds: req.user!.id }, $set: { updatedAt: new Date() } });
        await bumpWorldCacheVersion();
        const payload = { ok: true, alliance: null };
        res.json(payload);
    });
    app.post("/api/alliance/aid", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "alliance:aid", 20, 60_000))
            return;
    ...
    app.post("/api/alliance/aid/:id/claim", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "alliance:aid:claim", 30, 60_000))
            return;
        ...
        const now = new Date();
        await players.updateOne({ _id: req.user!.id }, {
            $set: { resources: nextResources, lastSeenAt: now },
            $inc: { allianceTroopReserve: aid.troops },
        });
        await allianceAids.updateOne({ _id: aid._id }, { $set: { status: "claimed", claimedAt: now } });
        res.json(await buildAllianceState(req.user!.id));
    });
    app.post("/api/game/clearings", requireAuth, async (req, res) => {
        ...
        return withPlayerMutationLock(req.user!.id, async () => {
            ...
            if (!isStarterClaim && !existing) {
                ...
                settlers = 10;
                const sourceTownIndex = towns.findIndex((town) => town?.id === sourceTownId ||
                    Math.hypot((town?.x ?? 0) - sourceX, (town?.y ?? 0) - sourceY) < 96);
                const sourceTown = normalizeTownSnapshotForState(sourceTownIndex >= 0
                    ? towns[sourceTownIndex]
                    : defaultTownSnapshotForTerritory(source.territory, req.user!.id, sourceTownId), req.user!.id, source.territory, now);
            ...
            if (!existing) {
                ...
                await Promise.all([
                    saves.updateOne({ playerId: req.user!.id }, { $set: { towns, updatedAt: now }, $setOnInsert: { _id: `save:${req.user!.id}`, playerId: req.user!.id, resources: DEFAULT_PLAYER_RESOURCES } }, { upsert: true }),
                    players.updateOne({ _id: req.user!.id }, { $set: { onboardingState: "claiming", lastSeenAt: now, resources: nextResources } }),
                ]);
            }
            ...
            res.json(payload);
            if (!existing) {
                void publishPlayerState(req.user!.id, "clearing_started", nextResources, towns).catch((error) => {
    ...
    app.post("/api/game/clearings/:id/complete", requireAuth, async (req, res) => {
        ...
        towns.push(town);
        await saves.updateOne({ playerId: req.user!.id }, {
            $set: { towns, updatedAt: now },
    ...
    app.delete("/api/game/clearings/:id", requireAuth, async (req, res) => {
        ...
        const save = (await saves.findOne({ playerId: req.user!.id }));
        const towns = Array.isArray(save?.towns) ? [...save.towns] : [];
        if ((clearing.settlers || 0) > 0 && clearing.sourceTerritoryId !== undefined) {
            const sourceTerritory = getStaticTerritory(clearing.sourceTerritoryId);
            if (sourceTerritory) {
                const sourceTownIndex = towns.findIndex((town) => town?.id === clearing.sourceTownId ||
                    Math.hypot((town?.x ?? 0) - sourceTerritory.x, (town?.y ?? 0) - sourceTerritory.y) < 96);
                if (sourceTownIndex >= 0) {
                    const sourceTown = normalizeTownSnapshotForState(towns[sourceTownIndex], req.user!.id, sourceTerritory, now);
                    sourceTown.population = Math.min(sourceTown.populationCapacity, sourceTown.population + Math.max(0, clearing.settlers || 0));
                    sourceTown.lastPopulationAt = now.toISOString();
                    towns[sourceTownIndex] = sourceTown;
                }
            }
    ...
    app.post("/api/game/marches/sources", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "game:march-sources", 80, 60_000))
            return;
        const parsed = MarchSourceOptionsSchema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: "bad_request", message: "Yêu cầu tìm thành xuất quân không hợp lệ" });
        const target = getStaticTerritory(parsed.data.toTerritoryId);
        if (!target)
            return res.status(404).json({ error: "not_found", message: "Không tìm thấy lãnh thổ mục tiêu" });
        const { territoryClaims, saves } = await collections();
        ...
        const sources = claims.map((claim) => {
            ...
            if (!territory)
                return null;
            const town = normalizeTownSnapshotForState(findTownForTerritory(towns, territory) || defaultTownSnapshotForTerritory(territory, req.user.id), req.user.id, territory);
            const route = resolveAttackRoute(territory, target);
            const connected = isClaimConnectedToCapital(claims, territory.id);
            const infantry = Math.max(0, Math.floor(Number(town.infantryCount || 0) || 0));
            const cavalry = Math.max(0, Math.floor(Number(town.cavalryCount || 0) || 0));
            const artillery = Math.max(0, Math.floor(Number(town.artilleryCount || 0) || 0));
            const troops = infantry + cavalry + artillery;
            const valid = connected && route.valid && troops > 0;
            const travel = calcTravelMetrics(territory, target, troops > 0 ? { infantry, cavalry, artillery } : { infantry: 1, cavalry: 0, artillery: 0 }, gameSettings, route.routeType === "sea");
            return {
                townId: town.id,
                territoryId: territory.id,
                valid,
                ...
                artillery,
                troops,
                reason: !connected ? "Thành đã bị cô lập khỏi Hoàng Thành" : troops <= 0 ? "Thành không còn quân sẵn sàng" : route.reason,
            };
        }).filter(Boolean);
        sources.sort((a, b) => Number(b.valid) - Number(a.valid) || a.travelSeconds - b.travelSeconds || b.troops - a.troops || a.territoryId - b.territoryId);
        res.json({
            ok: true,
    ...
    app.post("/api/game/marches", requireAuth, async (req, res) => {
        if (!enforceActionLimit(req, res, "game:march", 35, 60_000))
            return;
        const parsed = CreateMarchSchema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: "bad_request", message: "Lệnh hành quân không hợp lệ" });
        const from = getStaticTerritory(parsed.data.fromTerritoryId);
        const to = getStaticTerritory(parsed.data.toTerritoryId);
        if (!from || !to)
            return res.status(404).json({ error: "not_found", message: "Không tìm thấy lãnh thổ hành quân" });
        return withPlayerMutationLock(req.user!.id, async () => {
            const { territoryClaims, marchOrders, players, saves } = await collections();
            if (parsed.data.requestId) {
                const existingMarch = await marchOrders.findOne({ ownerId: req.user!.id, requestId: parsed.data.requestId });
                if (existingMarch) {
                    return res.json({ ok: true, duplicate: true, march: toPublicMarch(existingMarch) });
                }
            }
            ...
            if (parsed.data.kind === "attack") {
                ...
                if (!route.valid) {
                    const error = route.reason?.includes("Bến tàu") ? "source_port_required" : route.reason?.includes("nội địa") ? "target_not_coastal" : "target_not_on_frontier";
                    return res.status(409).json({ error, message: route.reason || "Không có tuyến tấn công hợp lệ" });
                }
            ...
            const save = (await saves.findOne({ playerId: req.user!.id }));
            const towns = Array.isArray(save?.towns) ? [...save.towns] : [];
            const sourceTownIndex = towns.findIndex((town) => town?.id === townIdForTerritory(from.id) ||
                Math.hypot((town?.x ?? 0) - from.x, (town?.y ?? 0) - from.y) < 96);
            const sourceTown = normalizeTownSnapshotForState(sourceTownIndex >= 0 ? towns[sourceTownIndex] : defaultTownSnapshotForTerritory(from, req.user!.id), req.user!.id, from);
            if (sourceTown.infantryCount < infantry ||
            ...
            sourceTown.troops = Math.max(0, sourceTown.infantryCount + sourceTown.cavalryCount + sourceTown.artilleryCount);
            if (sourceTownIndex >= 0)
                towns[sourceTownIndex] = sourceTown;
            else
                towns.push(sourceTown);
            ...
            await Promise.all([
                ...
                saves.updateOne({ playerId: req.user!.id }, {
                    $set: { towns, updatedAt: now },
                    $setOnInsert: { _id: `save:${req.user!.id}`, playerId: req.user!.id, resources: DEFAULT_PLAYER_RESOURCES },
                }, { upsert: true }),
            ]);
            ...
            res.json(payload);
            void publishPlayerState(req.user!.id, "march_created", null, towns, payload.newbieShieldUntil).catch((error) => {
    ...
    app.post("/api/game/recruit", requireAuth, async (req, res) => {
        return res.status(410).json({
            ...
            message: "Mộ binh thủ công đã được thay bằng bổ sung quân tự động theo đặc sản lãnh thổ",
        });
    ...
    app.post("/api/world/territories/:id/claim", requireAuth, async (req, res) => {
        ...
        const staticTerritory = buildStaticTerritoryList().find((territory: any) => territory.id === id);
        if (!staticTerritory) {
            return res.status(404).json({ error: "not_found", message: "Không tìm thấy lãnh thổ" });
        }
        const { players, territoryClaims, territoryClearings, alliances, saves } = await collections();
        ...
        const now = new Date();
        await territoryClaims.updateOne({ territoryId: id }, { $setOnInsert: { _id: `territory:${id}`, territoryId: id, playerId: req.user!.id, claimedAt: now } }, { upsert: true });
    ...
    app.post("/api/config", requireAuth, requireAdmin, async (req, res) => {
        ...
        const { configs } = await collections();
        const now = new Date();
        await configs.updateOne({ _id: "game_settings" }, { $set: { ...parsed.data, updatedAt: now } }, { upsert: true });
    ...
    app.post("/api/admin/territories/:id/reset", requireAuth, requireAdmin, async (req, res) => {
        const id = Number(req.params.id);
        if (isNaN(id))
            return res.status(400).json({ error: "bad_request", message: "ID không hợp lệ" });
    ...
    app.post("/api/admin/territories/:id/claim", requireAuth, requireAdmin, async (req, res) => {
        ...
        if (!player)
            return res.status(404).json({ error: "not_found", message: "Người chơi không tồn tại" });
        const staticTerritory = getStaticTerritory(id);
        if (!staticTerritory)
            return res.status(404).json({ error: "not_found", message: "Không tìm thấy lãnh thổ" });
        const now = new Date();
        await territoryClaims.updateOne({ territoryId: id }, {
